...
public class UpdateChecker {
	...
	static String getLatestRemote() {
		if (ChronoUnit.HOURS.between(lastCheck, Instant.now()) >= 4) {
			...
			if (versions.isPresent() && !versions.get().isEmpty()) {
				remote = versions.get().getFirst().versionNumber;
			}
			cachedLatest = remote;
	...
	static String getLocalVersion() {
		String version = "1000";
		var pkg = Winterly.class.getPackage();
		if (pkg != null) {
	...
	static Optional<ArrayList<ProjectVersion>> getModrinthVersions() {
		try {
			...
			Arrays.asList(versions).forEach(version -> {
				if (version.gameVersions.contains(SharedConstants.getCurrentVersion().name())) array.add(version);
			});
			...
			Winterly.LOG.info("Failed to parse modrinth project versions due to an exception:\n{}", String.valueOf(e));
		}
		return Optional.empty();
	}
